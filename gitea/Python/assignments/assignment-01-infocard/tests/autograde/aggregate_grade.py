...
编程测试满分 12（core 8 + edge 4），报告满分 3，总分 15
"""
...
def main():
    parser = argparse.ArgumentParser(description="Aggregate grades for assignment-01-infocard")
    ...
    prog_grade = load_json(args.programming, {"total_score": 0, "max_score": 12})
    report_grade = load_json(args.report, {"total": 0})

    prog_score = prog_grade.get("total_score", 0)
    prog_max = prog_grade.get("max_score", 12)
